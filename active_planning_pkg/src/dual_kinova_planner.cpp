...
class DualKinovaPlanner {
    ...
    std::string solve_strrt(double max_solve_time, double goal_travel_time, std::string group = "left_arm", double range=4, double batchSize=512, double timeBoundFactorIncrease=2)
    {
        spacetime_path.clear();
        ...
        for (int i = 0; i < goal_travel_times.size(); i++)
        {
            for (int j = 0; j < ranges.size(); j++)
            {
                for (int k = 0; k < initialBatchSizes.size(); k++)
                {
                    ...
                    {
                        results += std::to_string(ranges[j]) + " " + std::to_string(initialBatchSizes[k]) + " " + std::to_string(timeBoundFactorIncrease[l]) + " " +  nc.solve_strrt(30, goal_travel_times[i], "both_arms", ranges[j], initialBatchSizes[k], timeBoundFactorIncrease[l]) + ", " + std::to_string(nc.execute_path()) + "\n";
                        std::cout << "--------------------------" << std::endl;
                    }
                }
            }
